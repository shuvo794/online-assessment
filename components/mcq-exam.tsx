...
export function McqExam() {
  ...
  const [timedOut, setTimedOut] = useState(false);   // ← plain state, not ref
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // ── 1. Load exam on mount ─────────────────────────────────────────────────
  useEffect(() => {
    ...
    // Stamp startedAt once so refresh keeps correct countdown
    if (!data.startedAt) {
  ...
  // ── 2. Countdown — NEVER call router inside setState updater ──────────────
  useEffect(() => {
    if (secondsLeft === null || !exam) return;

    timerRef.current = setInterval(() => {
      // Just decrement — side-effects via separate effect below
  ...
  }, [exam, secondsLeft === null]); // start once exam+secondsLeft are ready

  // ── 3. React to secondsLeft reaching 0 — safe place for router.push ──────
  useEffect(() => {
    if (secondsLeft === 0 && exam && !timedOut) {
      if (timerRef.current) clearInterval(timerRef.current);
      setTimedOut(true);
      localStorage.removeItem("active_exam");
      router.push("/testTimeout");
    }
  }, [secondsLeft, exam, timedOut, router]);
  ...
  const handleSkip = () => {
    if (!exam) return;
    if (currentIdx < exam.questions.length - 1) setCurrentIdx((i) => i + 1);
  };
    ...
    <div className="mx-auto flex w-full max-w-[840px] flex-col gap-4 md:gap-6">
      ...
      <div className="flex h-16 items-center justify-between rounded-2xl border border-[#f0f0f0] bg-white px-5 shadow-sm md:h-[72px] md:px-6">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="max-w-[180px] truncate text-xs font-medium text-slate-400 md:max-w-xs">
            {exam.title}
          </span>
        </div>
        ...
        <div
          className={`flex items-center gap-2 rounded-xl px-5 py-2 text-lg font-bold transition-all md:px-7 md:py-2.5 md:text-xl ${
            isWarning
              ? "animate-pulse bg-red-50 text-red-600 ring-2 ring-red-200"
      ...
      <div className="flex flex-col gap-6 rounded-2xl border border-[#f0f0f0] bg-white p-5 shadow-sm md:gap-8 md:p-8">
              ...
              {question.options.map((option, index) => (
                ...
                >
                  <div className="relative flex items-center justify-center">
                    ...
                    />
                    <div className="size-5 rounded-full border border-slate-300 bg-white ring-offset-2 transition-all peer-checked:border-[5px] peer-checked:border-[#6633ff] md:size-6 md:peer-checked:border-[6px]" />
                  </div>
        ...
        {/* Action buttons */}
        <div className="flex flex-col gap-3 border-t border-slate-100 pt-5 md:flex-row md:items-center md:justify-between md:pt-4">
          <button
            type="button"
            onClick={handleSkip}
